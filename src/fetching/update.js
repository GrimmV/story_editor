...
export const uploadBackgroundImage = (token, id, image) => {
  const myHeaders = new Headers();
...
export const changeCharacterImgSrc = (token, id, imageSrc) => {
  ...
  return fetch(
    fullBasePath + "/characterframe/img/source/" + id,
    requestOptions
  )
    .then((response) => handleResponse(response))
    .then((response) => response.json())
...
export const addChoice = (token, id, frameId, substoryId, answerText) => {
  ...
  return fetch(
    fullBasePath + "/choices/add/" + tmpId + "/" + frameId + "/" + substoryId,
    requestOptions
  )
    .then(handleWrongFetchResult)
    .then((response) => response.json())
...
export const getCharacterImageCollection = () => {
  ...
  return fetch(
    fullBasePath + "/characterframe/available/images",
    requestOptions
  )
    .then(handleWrongFetchResult)
    .then((response) => response.json())
...
export const createSubstoryGPTinfo = (token, dto) => {
  const myHeaders = new Headers();
  myHeaders.append("Content-Type", "application/json");
  myHeaders.append("Authorization", "Bearer " + token);
  ...
  return fetch(`${fullBasePath}/substorygptinfo`, requestOptions)
    .then((response) => handleResponse(response))
    .then((response) => response.json())
    .catch((error) => console.log("error", error));
};

export const getSubstoryGPTinfo = (substoryId) => {
  const myHeaders = new Headers();

  return fetch(`${fullBasePath}/substorygptinfo/${substoryId}`, {
    headers: myHeaders,
  })
    .then((response) => handleResponse(response))
    .then((response) => response.json())
...
const postUpdate = (token, endpoint, substoryId, data) => {
  // possible endpoints = ["workarea", "employer", "employerinfo", "employee", "employeeInfo"]
  const myHeaders = new Headers();
  myHeaders.append("Content-Type", "text/plane");
  myHeaders.append("Authorization", "Bearer " + token);
  ...
  return fetch(
    `${fullBasePath}/substorygptinfo/${endpoint}/${substoryId}`,
    requestOptions
  )
    .then((response) => handleResponse(response))
    .then((response) => response.json())
...
export const updateWorkarea = (token, substoryId, workarea) =>
  postUpdate(token, "workarea", substoryId, workarea);
export const updateEmployer = (token, substoryId, employer) =>
  postUpdate(token, "employer", substoryId, employer);
export const updateEmployerInfo = (token, substoryId, employerInfo) =>
  postUpdate(token, "employerinfo", substoryId, employerInfo);
export const updateEmployee = (token, substoryId, employee) =>
  postUpdate(token, "employee", substoryId, employee);
export const updateEmployeeInfo = (token, substoryId, employeeInfo) =>
  postUpdate(token, "employeeinfo", substoryId, employeeInfo);
export const updateOutline = (token, substoryId, outline) =>
  postUpdate(token, "outline", substoryId, outline);
